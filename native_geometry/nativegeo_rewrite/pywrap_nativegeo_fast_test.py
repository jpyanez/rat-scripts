...
position *= (np.random.rand()*5900.)/np.linalg.norm(position)
print 'Producing at thsi position ', position
...
'/generator/vtx/set 10000 WAVELENGTH 0\n'+\
'/rat/procset file "OUTNAME.root"\n'+\
'/generator/pos/set ' +' '.join(["%.2f" % x for x in position]) + ' mm\n'+\
...
parser = OptionParser()
parser.add_option("-g", "--geometry", default="geo/snoplusnative.geo",
                  dest = 'geo')
parser.add_option("-o", "--outdir", default = 'nativegeo_buildtest',
                  dest="outdir")
...
parser.add_option("-w", "--wavelength", default = 380.,
                  dest='wl')
parser.add_option("-e", "--nevents", default = 40,
...
if __name__ == '__main__':
    ...
    outdir = '/home/jp/projects/snoplus/rundir/nativegeo_rewrite/' + options.outdir
